...
## Обновление от сессии DeepSeek №3 (20.09.2026) — читать первым

Загрузка сохранения починена и проверена. Корень краха `h3hota HD.exe+0x004C7C34` (read 0x0000000D) — не сейв и не установка HotA, а наш способ нажатия кнопки браузера загрузки (подмена vtable + синтетический подтип). Теперь все кнопки диалогов нажимаются адресным оконным mouse-событием в границах, которые контрол сообщает сам; новое: op48 `load:confirm`, op49 `game:main_menu`, op50 выход из браузера по элементу, op53 `load:back`, op26/29/30 кнопки вопросов (эксперимент с Enter/Escape снят).

Живая проверка 20.09: `menu:load` → `menu:single` → `load:select:1` → `load:confirm` → `completed`, `adventure`, игра жива, загружен AUTOSAVE (Лабета 139, 66,37, день 1, золото 10000) — совпадает с сейвом. Далее игра: рынок построен (5 дерева + 500 золота, `builtToday`), выход из города без падения, конец хода 1→2 (+1000 золота, сюжетное сообщение, автосейв), конец хода 2→3, собраны золото/дерево/самоцветы, костёр и сад откровения. На момент передачи: день 3, неделя 1, Лабета 139, золото 12700, дерево 19, самоцветы 8, движение обновлено новым днём, рынок в городе 5.

Обновлённые документы: `docs/knowledge/playbooks/save-load-cycle.md` (полная рабочая цепочка загрузки, что не работает и почему), `docs/knowledge/playbooks/town-economy.md` (метод выбора здания и каталог 18 слотов Conflux), `skills/hota-player/SKILL.md` (правило кнопок, признаки недостижимой цели в `move_to`), `docs/STATE.md` (сессия 3).

Открыто: подтверждение вопроса «ГЛАВНОЕ МЕНЮ» возвращает в партию вместо меню (обход — перезапуск кнопкой Play); внутриигровой `game:load` браузера не открывает (рабочий путь — через главное меню); ввод/чтение имени сейва не завершены. Сейвы `111ы.GM1`, `111.GM1`, `BATTLE.GM1` не тронуты, `AUTOSAVE.GM1` перезаписан самой игрой — это штатно.

## Обновление от сессии DeepSeek №2 — читать первым

Состояние: игра работает (PID 36792), наш сервер — `build/session-full/HotaMcp.exe --launcher-pid 47720`, HTTP `http://127.0.0.1:18773`. Коммиты этой сессии: `0ed8a10`, `63673d4`, `b93538b` (запушены).

Что исправлено и проверено живьём:

1. **Кнопки модальных диалогов.** Обработчик кнопки (`h3hota HD.exe` `0x00455E70`) сообщает о нажатии только при subtype `0xa`, а мост слал `0xd` — вопрос закрывался, действие не выполнялось. В `native/game/game_bridge.cpp`: `if(operation!=30)buttonMessage.subtype=0xa;`. После этого `message:accept`/`message:confirm` и подтверждение конца хода работают, игра не падает.
2. **Постусловия действий.** `completed` теперь требует смены `revision` наблюдаемого состояния; простое закрытие диалога больше не выдаётся за выполненное действие (`Bridge.Click`).
3. **Движение и сбор — работают.** `nearby_targets` → `move_to` по ресурсу: `completed`, герой 66,37 → 67,38, движение 1630 → 1389, золото 10000 → 10800.
4. **Важно:** `route.state = not_available` у цели НЕ означает недоступность — маршрут это предварительный просмотр; само перемещение считает игра. Не блокировать движение этим полем.
5. **Загрузка.** `game:load` в системных опциях открывает штатный вопрос, подтверждение проходит. Браузер загрузки открывается из главного меню: `menu:load` → `menu:single` (заголовок «Загрузить игру», кнопка `scnrlod.def` id186, выход `scnrback.def` id188). Загрузка на этой машине мгновенная — проверять её надо по различимому состоянию (позиция героя/ресурсы), а не по «экран сменился».
6. Город: `town:open:5` → `town` (`town:tavern`, `town:recruit:0..2`, `town:construction`, `town:close`) → `town:close` → `adventure`.

Открытые вопросы (следующему агенту):

- Кнопка «Главное меню» (`somain.def` id108) в системных опциях: действие `game:main_menu` зарегистрировано (аргумент 186, ожидание `main_menu`), но на живом прогоне операция ушла в `uncertain` и диалог остался открыт. Разобрать доставку именно этой кнопки.
- Клик по кнопке браузера загрузки (`scnrlod.def` id186) из нашего действия: диалог закрывается, игра возвращается к партии. Нужно подтвердить на различимом состоянии, что загрузка реально произошла, либо разобрать обработчик этой кнопки.
- Движение по `move_to` работает, но предварительный маршрут (`not_available`) не читается — если нужен показ стоимости пути, разбирать `RouteReader`/грид путеискателя (`[0x6992d4]+0x24` был NULL в этой партии).
- Сейвы: `111ы.GM1`, `111.GM1`/`AUTOSAVE.GM1`, `BATTLE.GM1` — целы, копия с хешами в `build/game-state-backup/`. Не удалять.

Ограничения сессии: без CUA и без перехвата мыши/клавиатуры; самодельные «клики» не изобретать — только действия костяка (`observe`, `act`/`click_ui` с ключами из `Actions`), настройки графики и меню пользователя без просьбы не менять.
